...
enum WallStatus checkWall(struct Game *game, int x, int y, char d)
{
    ...
    {
        case 'H':
            if (x < 0 || x >= game->dim - 1 || y < 0 || y >= game->dim) return INVALID_WALL;
        ...
        case 'V':
            if (y < 0 || y >= game->dim - 1 || x < 0 || x >= game->dim) return INVALID_WALL;
...
{   
    ...
    if (status != VALIDATE_WALL) return;
    ...
    for (int i = 0; i < 10; i++)
        for (int j = 0; j < 10; j++) visited[i][j] = 0;
...
/**
 * @details To prevent a conflict with other horizantal walls.
 */
int hasHorizontalWall(struct Game *game, int x, int y)
{
    if (x < 0 || x > game->dim || y < 0 || y > game->dim) return 1;
...
/**
 * @details To prevent a conflict with other Vertical walls.
 */
