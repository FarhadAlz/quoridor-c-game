...
void spellReduceWalls(struct Player *player)
{
    ...
    if (reduce > player->canwall)
        reduce = player->canwall;
...
void takeWalls(struct Player *player, struct Player *other)
{
    ...
    case 1:
        ...
        break;
    default:
        takenWalls = (rand() % 2) ? 1 : 2;
        break;
    }
    if (takenWalls > other->canwall)
        takenWalls = other->canwall;
...
void applyMagicEffect(struct Game *game, struct Player *current, struct Player *other)
{
    ...
    {
    case NEG_REMOVE_ALL_WALLS:
        spellRemoveAllWalls(game);
        break;
    case NEG_REDUCE_WALLS:
        spellReduceWalls(current);
        break;
    case NEG_BLOCK_PLAYER:
        spellBlockPlayer(current);
        break;
    case POS_INCREASE_WALLS:
        increaseWall(current);
        break;
    case POS_TAKEN_WALLS:
        takeWalls(current, other);
        break;
    default:
        break;
    }
